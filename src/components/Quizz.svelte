<script>
  ...
  const handleSubmit = () => {
    ...
    if (answer instanceof Array) result.isCorrect = answer.includes(userInput);
    else result.isCorrect = userInput === answer;
  ...
  const handleChange = (e) => {
    ...
    if (shouldFilter) userInput = userInput.replace(/[^A-Za-z]/g, "");
  };
...
<article class="quizz">
  <h2 class="quizz__kana --clickable" on:click={handleShowHint}>
    {word || "°"}
  </h2>
